...
type DemoPane struct {
	...
	// TODO: use data in app, not pane? - Just pass app to NewDemoPane, I think
//	a           *App
	isImageMode bool
	imageIndex  int
}
...
func (p *DemoPane) Gesture(gesture *gestic.GestureMessage) {
	...
	// check the second last touch location since the most recent one before a tap is
...
func (p *DemoPane) Render() (*image.RGBA, error) {
	...
	if p.isImageMode {
		...
		stateImg, ok := stateImages[stateImageNames[p.imageIndex]]
		if !ok {
		...
		drawText(fmt.Sprintf("%2d", p.imageIndex), color.RGBA{253, 151, 32, 255}, 2, img)
		drawText("OK!", color.RGBA{253, 151, 32, 255}, 9, img)
